...
Pipeline::Pipeline(std::vector<double> F, std::vector<double> OF, std::vector<double> B, std::vector<double> OB) {
    this->F = F;
    this->OF = OF;
    this->B = B;
    this->OB = OB;
    ...
    for (int i=0; i<OF.size(); i++) {this->TOF[i] = OF[i] / 100; this->TOB[i] = OB[i] / 100;}
