...
void Conductor::orchestrate(std::vector<int> all_num_machines, std::vector<double> all_bandwidths,
  ...
  auto compute_times           = g.compute_times;
  auto activation_sizes        = g.activation_sizes;
  auto parameter_sizes         = g.parameter_sizes;
  auto output_activation_sizes = g.output_activation_sizes;
  auto all_predecessor_ids     = g.all_predecessor_ids;
  ...
  for (int i = As.size() - 1; i >= 0; i--) {
    ...
    for (auto s : splits) {
      ...
      if (i == 0) {
        g.update_stage_id(end, stage_id);
      }
      stage_id += 1;
    }
