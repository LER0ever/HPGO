...
TypeA Conductor::compute_partitioning(d2d compute_times, d2d activation_sizes,
                                      d2d parameter_sizes,
                                      d2d output_activation_sizes,
                                      i2d all_predecessor_ids, int num_machines,
                                      int    num_machines_within_machine,
                                      double bandwidth, bool final_level) {
  TypeA A;
  ...
  for (int i = 0; i < compute_times.size(); i++) {
    for (int j = 0; j < compute_times[i].size(); j++) {
      ...
      for (int m = 0; m < max_m; m++) {
        ...
        double dp_comm_time = (2 * m * cur_parameter_size) /
                              (bandwidth * (m + 1));  // TODO: Bandwidth curve
        dp_comm_time /= num_machines_within_machine;
        if (cur_compute_time < 0.5)  // normally -1
          A[i][j][m] = std::make_tuple(-1, std::make_pair(-1, -1), -1);
        else
          A[i][j][m] =
              std::make_tuple((cur_compute_time + dp_comm_time) / (m + 1),
    ...
    for (int j = 0; j < A[i].size(); j++) {
      for (int m = 0; m < A[i][j].size(); m++) {
        ...
        std::cout << "A[" << i << "][" << j << "][" << m << "] = ("
                  ...
                  << opt_split.second << "), " << opt_num_machines << std::endl;
      }
    }
  }
}
